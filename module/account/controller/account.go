...
func GetAccountsFromUser(cpf_cnpj string) []entity_account.Account {
	...
	if err := db.GetGormDB().Table("tb_accounts").Where("cpf_cnpj = ?", cpf_cnpj).Find(&NewAccounts).Error; err != nil {
		return nil
	}
...
func AddAccountToDatabase(c *gin.Context, a *entity_account.Account) {
	...
	if err := db.GetGormDB().Table("tb_accounts").Create(&a).Error; err != nil {
...
func FindAccountInDatabase(c *gin.Context, as *[]entity_account.Account) {
	...
	if err := db.GetGormDB().Table("tb_accounts").Find(as).Error; err != nil {
		c.IndentedJSON(http.StatusInternalServerError, err)
		return
	}
...
func UpdateAccountInDatabase(c *gin.Context, a *entity_account.Account) {
	...
	if err := db.GetGormDB().Table("tb_accounts").Where("id = ?", a.Id).Updates(a).Error; err != nil {
		c.IndentedJSON(http.StatusInternalServerError, err)
		return
	}
...
func DeleteAccountInDatabase(c *gin.Context, a *entity_account.Account) {
	...
	if err := db.GetGormDB().Table("tb_accounts").Where("cpf_cnpj = ?", a.CpfCnpj).Delete(a).Error; err != nil {
		c.IndentedJSON(http.StatusInternalServerError, err)
		return
	}
...
func GetAccountById(id int) (entity_account.Account, error) {
	...
	if err := db.GetGormDB().Table("tb_accounts").Where("id = ?", id).First(NewAccount).Error; err != nil {
